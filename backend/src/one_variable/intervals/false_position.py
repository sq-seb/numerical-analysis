...
def false_position(*, f, a, b, niter, value, tol_type):
    ...
    while n <= niter:
        # 5.1 Compute x = xs - f_xs * (xs - xi) / (f_xs - f_xi)
        f_xi = f(xi)
        f_xs = f(xs)
        if f_xs == f_xi:
            return {"sol": None, "table": None, "error": f"Division by zero: f({xi}) = f({xs})"}
        x = xs - f_xs * (xs - xi) / (f_xs - f_xi)
    ...
    pd.options.display.float_format = pd.options.display.float_format = lambda x: "NaN" if pd.isna(x) else ("~0 (exceeds e-15)" if abs(x) < 1e-15 else f"{x:.14e}")
    table = pd.DataFrame({
        "xn" : x_list,
        "f(xn)" : f_list,
        "En" : E_list,
...
if __name__ == "__main__":
    ...
    result = false_position(f=f, a=0, b=1, niter=100, value=1e-7, tol_type="tol")
    print(result["table"])
    ...
    # Test 2
    result = false_position(f=lambda x: 77**(-x) * (-1 + x) + (x - 1)**(2 / 3) - 3, a=6.15, b=6.25, niter=100, value=10, tol_type="CS")
    print(result["table"])