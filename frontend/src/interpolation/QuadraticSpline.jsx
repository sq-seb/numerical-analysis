...
export default function QuadraticSpline() {
  ...
  const mutation = useMutation({
    mutationFn: async (payload) => {
      const res = await fetch(
        `${import.meta.env.VITE_API_URL}/quadratic_spline`,
        {
          method: "POST",
  ...
  const onSubmit = (data) => {
    ...
    // ✅ MUST HAVE AT LEAST 3 TOTAL POINTS
    if (n < 3) {
      return showError("At least 3 data points are required.");
    }

    const validationSize = Math.floor(n * vp);
    const trainingSize = n - validationSize;

    // ✅ MUST HAVE AT LEAST 3 TRAINING POINTS
    if (trainingSize < 3) {
      return showError(
        "Not enough training data (must have at least 3 training points)."
      );
    }
    ...
    <div className="w-full flex justify-center">
      <div className="w-full max-w-3xl">
        ...
        {mutation.data && (
          <div
            ref={resultRef}
            className="mt-4 p-5 bg-amber-400 text-black font-mono text-sm 
            rounded-2xl shadow-2xl shadow-black/50 
            animate__animated animate__zoomInDown"
          >
            {/* IMAGE */}
            {mutation.data.image && (
              <div className="mb-5 rounded-xl overflow-hidden shadow-inner">
                <img
            ...
            <div className="space-y-3 text-base leading-relaxed">
              {mutation.data.spline
                .split(" ; ")
                .map((segment, idx) => (
                  <div
                    key={idx}
                    className="bg-black/10 p-3 rounded-lg shadow-sm"
                  >
